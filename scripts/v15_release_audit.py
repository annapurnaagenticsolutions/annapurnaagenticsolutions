...
def main():
    ...
    if profile.get('release')!='1.5':errors+=fail('release profile must be 1.4')
    if profile.get('privacy',{}).get('localStorageKey')!='annapurnaLivingV15':errors+=fail('v1.5 local storage key mismatch')
    for fn in PAGES:
        path=ROOT/fn
    ...
    for marker in ['intent-chooser','why-view','adaptive-primary','adaptive-rationale','data-adaptive-block="proof"','data-adaptive-block="capabilities"','data-adaptive-block="living"']:
    ...
    for marker in ['annapurnaLivingV15','deriveIntent','reorderAdaptivePages','filterContent','navigateToSection','highlightElement','explainCurrentView','adjustDepth','AnnapurnaPageAgent','PerformanceObserver','data-live-count']:
        if marker not in js:errors+=fail(f'v1.5 runtime missing {marker}')
    if re.search(r"fetch\(['\"]https?://",js):errors+=fail('runtime adaptive/agent layer must not fetch remote endpoints')
    for script in ['interaction_contract_test.py','adaptive_contract_test.py','temporal_sensory_contract_test.py','comprehension_audit.py','performance_change_test.py']:
    ...
    if not errors:
        print('PASS v1.5 adaptive living release: 4 signals -> structure/depth/order/CTA')
        print('PASS embedded page agent: real page tools, rules-first, local-only, safe fallback')
        print('PASS shared temporal state + sensory choreography + reduced/low-power motion policy')
        print('PASS live-data transition + performance observation + 10% LCP/INP promotion policy')
        print('PASS four-page IA, privacy/history firewall and release budgets')
