...
def main():
    ...
    if profile.get('release')!='1.4':errors+=fail('release profile must be 1.4')
    if profile.get('privacy',{}).get('localStorageKey')!='annapurnaLivingV14':errors+=fail('v1.4 local storage key mismatch')
    for fn in PAGES:
        path=ROOT/fn
    ...
    for marker in ['annapurnaLivingV14','deriveIntent','reorderAdaptivePages','filterContent','navigateToSection','highlightElement','explainCurrentView','adjustDepth','AnnapurnaPageAgent','PerformanceObserver','data-live-count']:
        if marker not in js:errors+=fail(f'v1.4 runtime missing {marker}')
    if re.search(r"fetch\(['\"]https?://",js):errors+=fail('runtime adaptive/agent layer must not fetch remote endpoints')
    for script in ['interaction_contract_test.py','adaptive_contract_test.py','comprehension_audit.py','performance_change_test.py']:
    ...
    if not errors:
        print('PASS v1.4 adaptive living release: 4 signals -> structure/depth/order/CTA')
